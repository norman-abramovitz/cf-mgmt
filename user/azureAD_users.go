	...
	if m.AzureADConfig.Enabled {
		...
		for _, inputUser := range azureADUsers {
			...
			if uaaUser == nil {
				...
				// CreateExternalUser adds the created user (with GUID) to the
				// UAA manager's cached user list, which uaaUsers points at
				if err := m.UAAMgr.CreateExternalUser(userToUse.Upn, userToUse.Upn, userToUse.Upn, m.AzureADConfig.UserOrigin); err != nil {
